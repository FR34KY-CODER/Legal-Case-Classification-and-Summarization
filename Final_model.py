...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Summarization
# Importing necessary libraries for summarization
...
device = 0  # GPU (set -1 for CPU fallback)
summarizer = pipeline("summarization", model="facebook/bart-large-cnn", device=device)
# summarizer = pipeline("summarization", model="google/pegasus-xsum") 
# summarizer = pipeline("summarization", model="t5-base", tokenizer="t5-base", device=device)


# 3. Columns to summarize
columns_to_summarize = [
    "Fact", "Issue", "Petitioner's Argument", "Respondent's Argument",
    "Precedent Analysis", "Analysis of the law", "Court's Reasoning", "Conclusion"
]

# 4. Summarize section text
def summarize_section_text(text, section_name=""):
    if not isinstance(text, str) or text.strip() == "":
        return None
    try:
        input_length = len(text.split())
        if input_length < 20:
            return f"{section_name}: {text.strip()}"  # Don’t summarize short stuff
        output = summarizer(text[:1024], max_length=min(128, input_length + 30), min_length=5, do_sample=False)
        return f"{section_name}: {output[0]['summary_text']}"
...
# 5. Apply row-wise
def summarize_row_segments(row):
    summaries = []
    for col in columns_to_summarize:
        result = summarize_section_text(row.get(col, ""), section_name=col)
        if result:
            summaries.append(result)
    return "\n".join(summaries)

# 6. Generate the combined summary column
ik['summary_bart'] = ik.apply(summarize_row_segments, axis=1)

# 7. (Optional) View sample
print(ik[['summary_bart']].head(5).values[0][0])  # Preview first generated summary

import gc
import torch

# Clear Hugging Face model and tokenizer (if needed)
del summarizer
...
# 6. Generate the combined summary column
ik['summary_pegasus'] = ik.apply(summarize_row_segments, axis=1)

# 7. (Optional) View sample
print(ik[['summary_pegasus']].head(5).values[0][0])  # Preview first generated summary
...
# Load T5-based summarizer
# summarizer = pipeline("summarization", model="facebook/bart-large-cnn", device=device)
# summarizer = pipeline("summarization", model="google/pegasus-xsum") 
summarizer = pipeline("summarization", model="t5-base", tokenizer="t5-base", device=device)

# List of segmented columns to summarize
columns_to_summarize = [
    "Fact", "Issue", "Petitioner's Argument", "Respondent's Argument",
    "Precedent Analysis", "Analysis of the law", "Court's Reasoning", "Conclusion"
]
...
# Function to summarize individual section
def summarize_section_text(text, section_name=""):
    if not isinstance(text, str) or text.strip() == "":
        return None
    try:
        input_length = len(text.split())
        if input_length < 20:
            return f"{section_name}: {text.strip()}"  # Don't summarize short sections

        # Prefix required for T5
        t5_input = f"summarize: {text[:1024]}"
        output = summarizer(t5_input, max_length=min(128, input_length + 30), min_length=5, do_sample=False)
        return f"{section_name}: {output[0]['summary_text']}"
    except Exception as e:
        return f"[Error summarizing {section_name}] → {str(e)}"

# Function to summarize all sections in a row
def summarize_row_segments(row):
    summaries = []
    for col in columns_to_summarize:
        result = summarize_section_text(row.get(col, ""), section_name=col)
...
# Apply summarization to your DataFrame (ik should be loaded before)
ik['summary_t5'] = ik.apply(summarize_row_segments, axis=1)

# Preview
print(ik[['summary_t5']].head(5).values[0][0])
...
ik.to_csv(r'C:\Users\FR34K\Desktop\Coding\BTP\Final_Summaries.csv', index=False)
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~